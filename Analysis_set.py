...
change on 5/7/2023 - streamline to use in colab; old version is a memory hog
"""

import intg_support.Table_manager as c_tab
import pandas as pd
import os
from intg_support.file_handlers import save_df, get_df
#import zipfile
#import datetime
...
#          = self.t_man.tables['disclosures'].set_index('UploadKey')
#         self.df = self.t_man.tables['disclosures'].set_index('UploadKey').\
#                     join(self.t_man.tables['chemrecs'].set_index('UploadKey'),
#                          on='UploadKey',
#                          how='inner') #,validate='1:m')
#         # print(f'df {len(self.df)}' )
#         self.df = self.df.set_index('bgCAS').\
#                     join(self.t_man.tables['bgCAS'].set_index('bgCAS'),
#                          on='bgCAS',
